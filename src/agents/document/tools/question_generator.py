...
JSON Array:"""
    ...
    async def _generate_questions_for_difficulty_async(
        self,
        content: str,
        difficulty: str,
        count: int,
    ) -> List[Dict]:
        """Generate questions for a single difficulty level asynchronously.
...
JSON Array:"""
        ...
        try:
            llm = self._get_llm()
            # Use native async LLM invocation
    ...
    async def _generate_parallel_async(
        ...
        Uses native async LLM calls for better performance.
        """
        import asyncio
        ...
        # Run parallel async generation
        coroutines = [
            self._generate_questions_for_difficulty_async(content, difficulty, count)
            for difficulty, count in tasks
            if count > 0
        ]
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                difficulty = tasks[i][0]
                logger.error(f"Parallel generation failed for {difficulty}: {result}")
            else:
                all_questions.extend(result)

        parallel_time = (time.time() - start_time) * 1000
        logger.info(
            f"Async parallel generation: {len(all_questions)} questions in {parallel_time:.1f}ms"
        )
    ...
    async def _arun(
        ...
        """Generate comprehension questions from document content asynchronously.

        Uses native async LLM invocation for better performance.
        """
        start_time = time.time()
        ...
        # Use async parallel generation for better performance
        try:
            questions = await self._generate_parallel_async(content, num_questions)
